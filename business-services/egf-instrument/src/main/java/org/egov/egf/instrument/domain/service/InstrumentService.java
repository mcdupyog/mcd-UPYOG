...
public class InstrumentService {
	...
	public InstrumentService(SmartValidator validator, InstrumentRepository instrumentRepository,
			BankContractRepository bankContractRepository,
				...
				for (Instrument instrument : instruments) {
					...
					case "cash":
						if (instrument.getTransactionNumber() == null)
							throw new InvalidDataException("TransactionNumber(Cash)", ErrorCode.NOT_NULL.getCode(),
									null);
						// Assert.isTrue(DateUtils.isSameDay(instrument.getTransactionDate(),
						// Calendar.getInstance().getTime()),
						// "Cash Transaction Date must be current date");
						// Assert.notNull(instrument.getPayee(), "Payee Details for Cash Transaction
						// must not be null");
						break;
					case "cheque":
						if (instrument.getTransactionNumber() == null)
							throw new InvalidDataException("TransactionNumber(Cheque)", ErrorCode.NOT_NULL.getCode(),
									null);
						// Assert.notNull(instrument.getPayee(), "Cheque Payee Details must not be
						// null");
					...
					case "dd":
						...
						cal1.setTime(instrument.getTransactionDate());
						// Assert.isTrue(cal1.after(cal), "DD Transaction should be before 6 months of
					...
					case "online":
						if (instrument.getTransactionNumber() == null)
							throw new InvalidDataException("TransactionNumber(Online)", ErrorCode.NOT_NULL.getCode(),
									null);
						// Assert.notNull(instrument.getPayee(), "Online Payee Details must not be
						// null");
						// cal1.setTime(instrument.getTransactionDate());
						// Assert.isTrue(cal1.after(cal), "DD Transaction should be before 6 months of
						// current date or a future
					...
					case "bankchallan":
						if (instrument.getTransactionNumber() == null)
							throw new InvalidDataException("TransactionNumber(BankChallan)",
									ErrorCode.NOT_NULL.getCode(), null);
						if (instrument.getBank() == null)
							throw new InvalidDataException("BankDetails(BankChallan)", ErrorCode.NOT_NULL.getCode(),
									null);
						if (instrument.getBankAccount() == null)
							throw new InvalidDataException("BankAccountDetails(BankChallan)",
									ErrorCode.NOT_NULL.getCode(), null);
						// Assert.notNull(instrument.getPayee(), "Bank Challan Payee Details must not be
						// null");
						// cal1.setTime(instrument.getTransactionDate());
						// Assert.isTrue(cal1.after(cal), "DD Transaction should be before 6 months of
						// current date or a future
	...
	public List<Instrument> fetchRelated(List<Instrument> instruments, RequestInfo requestInfo) {
		...
		if (instruments != null)
			for (Instrument instrument : instruments) {
				 ...
				 * instrument.getBank().setTenantId(instrument.getTenantId()); BankContract bank
				 * = bankContractRepository.findById(instrument.getBank(), requestInfo); if
				 * (bank == null) throw new InvalidDataException("bank", "bank.invalid",
				 * " Invalid bank"); instrument.setBank(bank); }
				 */
				/*
				 * if (instrument.getBankAccount() != null &&
				 * instrument.getBankAccount().getAccountNumber() != null) {
				 ...
				 * .findByAccountNumber(instrument.getBankAccount(), requestInfo); if
				 * (bankAccount == null) throw new InvalidDataException("bankAccount",
				 * "bankAccount.invalid", " Invalid bankAccount");
...
//                }
				...
				// Reason for instrument surrender is not initiated by collection service so
				// that for now we are commenting out the validation from this service.
				 ...
				 * .findById(instrument.getSurrenderReason()); if (surrenderReason == null)
				 * throw new InvalidDataException("surrenderReason", "surrenderReason.invalid",
				 * " Invalid surrenderReason"); instrument.setSurrenderReason(surrenderReason);
				 * }
				 */
